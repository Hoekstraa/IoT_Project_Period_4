...
// 1 Addr 10806062 unit 13 on/off, period: 257us   replace with your own code
// 2 Addr 10806062 unit 14 on/off, period: 257us
// 3 Addr 10806062 unit 15 on/off, period: 257us

// Supported KaKu devices -> find, download en install corresponding libraries
#define unitCodeApa3      10806062  // replace with your own code
...
#define triggerPin      6  // output, always HIGH
...
{   
   apa3Transmitter.sendUnit(13, state);          // APA3 Kaku (Gamma)                
   delay(100);
}
...
{     
         ...
         case 'h': // Report sensor value to the app  
            intToCharBuf(sonar.ping_cm(), buf, 4);                // convert to charbuffer
            server.write(buf, 4);                             // response is always 4 chars (\n included)
            Serial.print("Sensor: "); Serial.println(buf);
            break;
